---
name: commit_chinese
description: "当用户要按功能链路执行中文 Git 提交时使用，例如“中文commit”“日常commit”“帮我拆分提交”。它只处理当前工作区内允许访问的 Git 已知改动，按功能流安全拆分并执行中文 Conventional Commit；不用于修改代码、处理未跟踪文件或纳入受限路径。成功标准是按独立主题完成拆分提交，必要时补充正文，并汇总 commit 结果与总改动行数。"
---
...
- 不同的平级功能、skill、模块，默认视为独立分组，即使它们都属于文档、提示词或元数据更新。
...
- 文档、示例、提示词、测试、元数据，只有在直接支撑同一功能链路时，才能与代码一起提交；若引入第二个主题，必须拆分。
- README、仓库说明、跨 skill 文档，除非只描述同一个单一功能，否则不能混入功能提交。
...
- 当一条提交包含同一主线下的多个协同子改动时，必须补正文，且每个点都要描述一个具体子变更。
- 像 `chore: 同步多个 skill 描述` 这种标题，如果覆盖了多个平级对象，仅写标题是不够的；要么拆分提交，要么在正文中逐条写清楚。
- 如果正文里的列点已经开始描述多个独立意图，就说明这条提交拆分不够，应继续拆。
...
- 提交前：明确检查每个计划中的 commit 是否只包含一个独立主题。
- 提交前：明确判断每个计划中的 commit 是标题即可，还是必须补正文。
