...
## 核心规范
- 部署脚本使用 `docker` 部署, 构造前自动停止/清理旧容器, 镜像重启策略为 `restart: always`， deploy 相关文件位于项目 `deploy` 下
